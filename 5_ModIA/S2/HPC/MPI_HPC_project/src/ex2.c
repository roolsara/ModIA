...
void bcast_A(int p, int q, Matrix *A, int i, int l, MPI_Comm row_comm) {
  // je ne suis plus propriétaire d'un bloc 
  // ça concerne tous les blocs d'une ligne 
    ...
    int b = A->b;
    Ail =  & A->blocks[i][l];
    /* Transmit A[i,l] using MPI_Bcast */

    if (q > 1 && my_row == Ail -> row /* Ail is stored on my row */) {
      if (Ail->owner != me) {
...
void bcast_B(int p, int q, Matrix *B, int l, int j, MPI_Comm col_comm) {
    ...
    int b = B->b;
    Blj =  & B->blocks[l][j];
    /* Transmit B[l,j] using MPI_Bcast */
    if (p > 1 && my_col == Blj -> col /* Blj is stored on my column */) {
      if (Blj->owner != me) {
