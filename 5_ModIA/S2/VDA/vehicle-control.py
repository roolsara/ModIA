...
def objective(u_hat):
    ...
    for k in range(1,2*P+1): # k varies in t_hat, the current working intervall
        ...
        if k>P: # 2nd half of intervall = prediction intervall
            ...
            #Jreg_val[k] = (u_hat[k]-u_hat[k-1])**2 # MAY BE CHANGED #penalisation de la dérivée première
...
P = 40; print('The prediction intervall length (index unit) P=',P)
M = 30; print('The control horizon index  M=',M,'. One must have: M <= P')

# Regularization parameter
alpha_du = 0.001; print('The regularization weight parameter: alpha_du=',alpha_du) # MAY BE CHANGED
