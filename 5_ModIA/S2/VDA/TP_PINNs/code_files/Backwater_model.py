...
def J_res(model, col, domain):
    ...
    h = model(domain).clamp(min = 1e-3)

    j = (q**2)/((Ks**2)*(h)**(10/3))
    Fr = (q**2)/(g*h**3)
    # h.backward(torch.ones_like(h), retain_graph=True)  
    h_prime = torch.autograd.grad(h, domain, torch.ones_like(h), create_graph =True, retain_graph =True)[0]
    
    return torch.square(torch.norm(h_prime + (b_prime + j )/(1-Fr)))/N_col
...
def J_obs(model, obs):
    ...
    return torch.square(torch.norm(model(obs.data[:,0].view(-1, 1)) - obs.data[:,1].view(-1,1)))/obs.N_obs
