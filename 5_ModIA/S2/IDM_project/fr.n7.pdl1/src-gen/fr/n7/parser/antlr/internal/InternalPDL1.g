...
@after {
	...
	(
		...
		{
			$current = $this_Ressources_3.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
entryRuleUseRessources returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getUseRessourcesRule()); }
	iv_ruleUseRessources=ruleUseRessources
	{ $current=$iv_ruleUseRessources.current; }
	EOF;

// Rule UseRessources
ruleUseRessources returns [EObject current=null]
@init {
	enterRule();
}
@after {
