...
import numpy as np 
import manage_exp as me
import lin2d_exp as le
import time 
...
class Lin2d(nn.Module):
    ...
    def __init__(self, x_dim, N, dt, init,
        ...
        nn.Module.__init__(self)
        # implement M
        theta = torch.tensor(torch.pi/100)
        self.M = torch.tensor([[torch.cos(theta), torch.sin(theta)],
                           [-torch.sin(theta), torch.cos(theta)]])


        self.x0 = me.get_x0(le.b_size, x_dim, le.sigma0)
...
class Gaussian(nn.Module):
    ...
    def __init__(self, x_dim, vec, inds):
        ...
        identity = torch.eye(x_dim, device=vec.device).expand(bs, x_dim, x_dim)
        self.cov_inv_ = torch.linalg.solve_triangular(self.Lambda, identity, upper=False)
        self.cov_log_det_ = torch.sum( logcst + diaga, dim=1).unsqueeze(1)
        # end = time.time()
        # print("Temps code amélioré : ", end - start)
        ...
        # end = time.time()
        # print("Temps code de base : ", end - start)
    ...
    def log_prob(self, x):
        ...
        mean_diff = x - self.mu
        L = self.Lambda
        z = torch.linalg.solve_triangular(L, mean_diff.unsqueeze(-1), upper=False)
        logprob = -0.5 * torch.bmm(z.transpose(1, 2), z).squeeze() - self.cov_log_det_
